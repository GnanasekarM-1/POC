trigger APL_UpdateIBForContract on SVMXC__Service_Contract__c (after update) 
{
    ...
    if(!.isTriggerEnabled('SVMXC__Service_Contract__c',userInfo.getUserId(),userInfo.getProfileId())){
        System.debug(.WARN,'APL_UpdateIBForContract execution is skipped.');
        return;
    }
    boolean runTrigger =  ;
    public Map< ,  > d   =   Map<String,String>();
    public SVMXC.    = new SVMXC.COMM_Utils_ManageSettings();
    svmxSettingList =  .SVMX_getSettingList('GLOB001');
